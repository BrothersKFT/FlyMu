...
__declspec(naked) void FixAttackSpeed()
{
    ...
    if( CLASS == DarkKnight || CLASS == BladeKnight || CLASS == BladeMaster){
		if(gObjUser.MagickAttack == SKILL_DEATH_STAB || gObjUser.MagickAttack == SKILL_TWISTING_SLASH )
		{
			if(STR_SPEED > 1584)
			{
				STR_SPEED = 1584;
    ...
    /////////////////////////////////////////////////
    /*======UPPERCUT/CYCLONE/LUNGE/FALLING SLASH FIX ALL CLASSES======*/	
	/////////////////////////////////////////////////

    if( CLASS == DarkLord || CLASS == LordEmperor || CLASS == DarkKnight || CLASS == BladeKnight || CLASS == BladeMaster || CLASS == MagicGladiator || CLASS == DuelMaster){
        if(gObjUser.MagickAttack == SKILL_UPPERCUT || gObjUser.MagickAttack == SKILL_CYCLONE || gObjUser.MagickAttack == SKILL_FALLING_SLASH || gObjUser.MagickAttack == SKILL_LUNGE)
        {
            if(STR_SPEED > 1000)
            {
                STR_SPEED = 1000;
